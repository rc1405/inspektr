    ...
    fn catalog(&self, files: &[FileEntry]) -> Result<Vec<Package>, CatalogerError> {
        ...
        for file in files {
            ...
            let parsed = match file_name {
                "pom.xml" => { if let Some(t) = file.as_text() { parse_pom_xml(t)? } else { continue; } }
                "build.gradle" | "build.gradle.kts" => { if let Some(t) = file.as_text() { parse_build_gradle(t)? } else { continue; } }
                _ => continue,
            };
...
pub fn parse_pom_xml(content: &str) -> Result<Vec<Package>, CatalogerError> {
    ...
    while let Some(start) = content[search_from..].find("<dependency>") {
        ...
        let end = match block_content.find("</dependency>") { Some(e) => e, None => break };
...
pub fn parse_build_gradle(content: &str) -> Result<Vec<Package>, CatalogerError> {
    ...
    for line in content.lines() {
        ...
        for config in &dep_configs {
            let rest = if let Some(r) = trimmed.strip_prefix(config) { r.trim() } else { continue; };
...
mod tests {
  ...
  <dependencies>
    ...
    <dependency>
      ...
      <version>4.13.2</version>
        ...
        assert!(pkgs.iter().any(|p| p.name == "org.springframework:spring-core" && p.version == "5.3.21"));
        assert!(pkgs.iter().any(|p| p.name == "junit:junit" && p.version == "4.13.2"));
    }
  ...
  <dependencies>
    ...
    api 'com.google.guava:guava:31.1-jre'
        ...
        assert!(pkgs.iter().any(|p| p.name == "org.springframework:spring-core" && p.version == "5.3.21"));
        assert!(pkgs.iter().any(|p| p.name == "junit:junit" && p.version == "4.13.2"));
        assert!(pkgs.iter().any(|p| p.name == "com.google.guava:guava" && p.version == "31.1-jre"));
    }
  ...
  <dependencies>
    <dependency>
      ...
      <version>1.0.0</version>
        ...
        assert_eq!(pkgs[0].metadata.get("source").map(|s| s.as_str()), Some("pom.xml"));
    }
